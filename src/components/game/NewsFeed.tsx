          ...
          currentWeekNews.map((m) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-[12px] text-foreground/80 leading-tight font-medium">{getMessageDisplayContent(m, getTeamName)}</p>
                {bid && onOpenPlayerContract && (
                  <Button onClick={(e) => { e.stopPropagation(); onOpenPlayerContract(bid.playerId); }} variant="outline" size="sm" className="mt-2 h-8 text-[10px] font-black uppercase border-primary/50 bg-primary/10 hover:bg-primary/30 text-primary rounded-lg">View contract</Button>
                )}
              </div>
            );
          })
